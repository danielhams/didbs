#
GBS0_INSTALL_DIR="$INSTALLDIR/gbs0"
GBS1_INSTALL_DIR="$INSTALLDIR/gbs1"

# Don't use

CC="$DIDBS_CC"
CXX="$DIDBS_CXX"
#CPPFLAGS="-I$INSTALLDIR/include"
#CFLAGS="-O1 -mips4"
#CXXFLAGS="-O1 -mips4"
#LDFLAGS="-L$INSTALLDIR/lib"
PERL="$INSTALLDIR/bin/perl"
TEST_SHELL="$INSTALLDIR/bin/bash"
SHELL="$INSTALLDIR/bin/bash"
CONFIG_SHELL="$INSTALLDIR/bin/bash"
PATH="$GBS0_INSTALL_DIR/bin:$PATH"
GBS0_LD=$GBS0_INSTALL_DIR/bin/ld
GSB0_AS=$GBS0_INSTALL_DIR/bin/as
# Linux
#PATH="$PATH"
#GBS0_LD=/usr/bin/ld
#GSB0_AS=/usr/bin/as
export CC CXX CPPFLAGS CFLAGS CXXFLAGS LDFLAGS PERL TEST_SHELL SHELL CONFIG_SHELL GBS0_INSTALL_DIR GBS1_INSTALL_DIR PATH GBS0_LD GBS0_AS
