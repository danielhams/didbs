...
CC="$DIDBS_CC"
CXX="$DIDBS_CXX"
CPPFLAGS="$DIDBS_MP_CPPFLAGS -I$INSTALLDIR/include/libdicl-0.1 -I$INSTALLDIR/include"
if [ "ne$DIDBS_CC" == "negcc" ]; then
#    CFLAGS="$DIDBS_ARCH_CFLAGS $DIDBS_GCC_OPT_SWITCH -fpermissive"
    CFLAGS="$DIDBS_ARCH_CFLAGS $DIDBS_GCC_OPT_SWITCH"
else
    CFLAGS="$DIDBS_ARCH_CFLAGS -O1"
fi
CXXFLAGS="$CFLAGS"
LDFLAGS="$LDFLAGS -L$INSTALLDIR/$DIDBS_LIBDIR -ldicl-0.1"
