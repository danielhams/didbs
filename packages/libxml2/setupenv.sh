...
if [ "ne$DIDBS_CC" == "negcc" ]; then
    ...
    LDFLAGS="$DIDBS_ARCH_LDFLAGS -L$INSTALLDIR/$DIDBS_LIBDIR -liconv"
else
    CFLAGS="$DIDBS_ARCH_CFLAGS $DIDBS_O3_NOIPA_CFLAGS $DIDBS_NOWARN_CFLAGS"
    LDFLAGS="$DIDBS_ARCH_LDFLAGS $DIDBS_NOIPA_LDFLAGS $DIDBS_NOWARN_LDFLAGS -L$INSTALLDIR/$DIDBS_LIBDIR -liconv"
fi
CXXFLAGS="$CFLAGS"
...
if [ "ne$DIDBS_LIBDIR" == "nelib32" ]; then
    ...
    export LD_LIBRARYN32_PATH PKG_CONFIG_PATH LD_LIBRARY_PATH
else
    LD_LIBRARYN64_PATH="$INSTALLDIR/lib64:$LD_LIBRARYN64_PATH"
    PKG_CONFIG_PATH="$INSTALLDIR/lib64/pkgconfig:$PKG_CONFIG_PATH"
    LD_LIBRARY_PATH="$LD_LIBRARYN64_PATH"
    export LD_LIBRARYN64_PATH PKG_CONFIG_PATH LD_LIBRARY_PATH
fi
